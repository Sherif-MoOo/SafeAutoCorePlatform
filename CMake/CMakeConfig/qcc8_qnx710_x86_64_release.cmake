...
# 2. Architecture-specific x86_64/SSE/AVX optimizations
...
#[=======================================================================[
.rst:
QNX710_X86_64_QCC8_MAXIMUM_PERFORMANCE
----------------------------------------
Maximum performance configuration for QNX 7.1.0 x86_64 using QCC-8 (GCC 8.3.0)
...
.. code-block:: bash
    ...
    cmake -C qcc8_qnx710_x86_64_release.cmake \
...
string(APPEND OPENAA_C_FLAGS " -msse4.2")
...
#
# -march=x86-64: Base x86_64 architecture
#   • Targets AMD K8/Intel Core 2 baseline
#   • Ensures compatibility with all x86_64 processors
#   • Includes: SSE, SSE2 (mandatory for x86_64)
#   • Example: Uses MOVSD for double operations vs x87 FLD/FST
#
# -mtune=generic: Optimize for common x86_64 processors
#   • Balances performance across Intel/AMD architectures
#   • Tunes for: Sandy Bridge, Haswell, Zen characteristics
#   • Example: Instruction scheduling for 3-4 wide decode
#   • Better than no tuning: 5-10% improvement typical
#
# -msse4.2: Enable SSE through SSE4.2
#   • SSE2: 16 XMM registers, double precision SIMD
#   • SSE3: Horizontal adds (HADDPS), complex arithmetic
#   • SSSE3: Byte permutation, absolute values
#   • SSE4.1: Dot products, blending, rounding
#   • SSE4.2: String processing, CRC32 acceleration
#   • Example: PCMPESTRI for fast string comparisons
#   • Real code: _mm_crc32_u64() → CRC32 instruction
#
# -mfpmath=sse: Use SSE for floating-point math
#   • Performance: 2-3x faster than x87 for scalar FP
#   • Precision: IEEE 754 compliant (unlike x87 80-bit)
#   • Example: All float/double math uses XMM registers
#   • Benefits: No x87 stack bottlenecks, better scheduling

# ╔════════════════════════════════════════════════════════════════════╗
# ║        ADVANCED SIMD EXTENSIONS (Performance Features)             ║
# ╚════════════════════════════════════════════════════════════════════╝

string(APPEND OPENAA_C_FLAGS " -mpopcnt")
string(APPEND OPENAA_C_FLAGS " -mcx16")
string(APPEND OPENAA_C_FLAGS " -msahf")
string(APPEND OPENAA_C_FLAGS " -mmovbe")
string(APPEND OPENAA_C_FLAGS " -maes")
string(APPEND OPENAA_C_FLAGS " -mpclmul")

# Advanced instruction set details:
# -mpopcnt: Population count instruction
#   • Performance: Bit counting in 1 cycle vs 10-20
#   • Example: __builtin_popcount() → POPCNT instruction
#   • Use case: Hamming distance, bit manipulation
#   • Real impact: 10x faster for sparse matrix operations
#
# -mcx16: CMPXCHG16B - 128-bit compare and swap
#   • Performance: Lock-free 128-bit atomics
#   • Example: std::atomic<__int128> operations
#   • Critical for: ABA-problem free data structures
#   • Alternative: Requires locks without this
#
# -msahf: SAHF/LAHF in 64-bit mode
#   • Performance: Faster flag register manipulation
#   • Example: Optimizes floating-point comparisons
#   • Legacy: Improves x87 → SSE transitions
#   • Impact: 2-3% for mixed FP code
#
# -mmovbe: Move with byte swap (big endian)
#   • Performance: Network byte order in 1 instruction
#   • Example: __builtin_bswap32() → MOVBE
#   • vs manual: Saves 2-3 instructions per swap
#   • Network code: 5-10% improvement
#
# -maes: AES-NI hardware encryption
#   • Performance: 10-100x faster AES operations
#   • Example: Single round in 1 cycle vs 10-20
#   • OpenSSL: 50MB/s → 1.5GB/s for AES-128-CBC
#   • Real code: _mm_aesenc_si128() → AESENC
#
# -mpclmul: Carry-less multiplication
#   • Performance: GCM mode 5-10x faster
#   • Example: Galois field ops in 1 cycle
#   • CRC: Alternative fast implementation
#   • Crypto: Essential for AES-GCM performance
...
# x86_64 SSE/AVX vectorization details:
...
#     // Vectorized SSE: 4 vector loads, 4 vector adds, 4 vector stores
#     // Compiles to: MOVAPS xmm0, [rsi]; ADDPS xmm0, [rdi]
...
#     // Compiles to single: ADDPS xmm0, xmm1
#
...
#       process(arr[i]);
#     }
#   • Tuned for: x86_64's 64-byte cache lines, 100-200 cycle memory latency
#
...
#   • Most effective on older in-order cores
#
...
#   • x86_64 I-cache: 64-byte lines, critical for hot paths
...
#   • x86_64: 32-byte fetch window typical
#
...
string(APPEND OPENAA_C_FLAGS " -minline-stringops-dynamically")
string(APPEND OPENAA_C_FLAGS " -momit-leaf-frame-pointer")
string(APPEND OPENAA_C_FLAGS " -fno-plt")

# x86_64 specific optimization details:
# -maccumulate-outgoing-args: Pre-allocate stack space
#   • Performance: Avoids stack pointer adjustments
#   • Example: All calls use same stack frame
#   • Trade-off: Slightly larger stack usage
#   • Benefit: 2-5% for call-heavy code
#
# -minline-all-stringops: Inline string operations
#   • Performance: Avoids PLT calls to libc
#   • Example: memcpy → REP MOVSB inline
#   • Small copies: 10x faster (no call overhead)
#   • Large copies: Falls back to library
#
# -minline-stringops-dynamically: Smart string inlining
#   • Performance: Runtime size checks
#   • Example:
#     if(n < 64) { inline_memcpy(); } else { memcpy@plt(); }
#   • Best of both: Fast for small, efficient for large
#
# -momit-leaf-frame-pointer: Omit FP in leaf functions
#   • Performance: Extra register in leaf functions
#   • Leaf function: Doesn't call other functions
#   • Impact: 50%+ of functions are leaves
#   • Benefit: 1-2% overall improvement
#
# -fno-plt: Avoid PLT for external calls
#   • Performance: Direct GOT calls
#   • Example: printf@plt → *printf@GOT
#   • Saves: 1 indirect jump per call
#   • Trade-off: Slightly larger code
...
#
# -fcf-protection=branch: Intel CET branch protection
#   • Purpose: Control flow integrity for indirect branches
#   • Hardware: Requires Intel Tiger Lake or newer
#   • Performance: <2% overhead with hardware support
#   • Fallback: NOP on older processors
#
# -mshstk: Shadow stack support
#   • Purpose: Hardware-protected return addresses
#   • Works with: -fcf-protection for full CET
...
string(APPEND OPENAA_C_FLAGS " --param prefetch-latency=100")
...
#   • x86_64: More registers allow larger inlines
...
#   • x86_64: Complex instructions justify more growth
#
...
string(APPEND OPENAA_C_FLAGS_RELEASE " -flto=auto")
...
string(APPEND OPENAA_CXX_FLAGS_RELEASE " -flto=auto")
...
#
# -mavx/-mavx2: Advanced Vector Extensions
#   • Status: Not enabled for generic x86_64 compatibility
#   • Performance: 2x wider vectors (256-bit)
#   • Requirement: Sandy Bridge (AVX) or Haswell (AVX2)
#   • Alternative: Use runtime detection with function multiversioning
#
# -mfma: Fused Multiply-Add
#   • Status: Not enabled for compatibility
#   • Performance: a*b+c in single operation
#   • Requirement: Haswell or newer
#   • Benefit: 2x throughput for linear algebra
#
# -mbmi/-mbmi2: Bit Manipulation Instructions
#   • Status: Not enabled for compatibility
#   • Examples: TZCNT, LZCNT, PDEP, PEXT
#   • Requirement: Haswell or newer
#   • Use case: Bit-field manipulation
#
# -march=native: Optimize for build machine
#   • Status: Not used for portability
#   • Alternative: -march=x86-64 for compatibility
#   • Option: Enable for site-specific builds
#
# -mtune=intel/-mtune=amd: Vendor-specific tuning
#   • Status: Using -mtune=generic
#   • Balances: Performance across Intel/AMD
#   • Alternative: Site-specific tuning possible
#
# -mindirect-branch=thunk: Spectre v2 mitigation
#   • Status: Available but not enabled
#   • Performance: 5-30% overhead
#   • Alternative: Retpoline for critical code
#
...
message(STATUS "- Architecture: x86-64 + SSE4.2")
