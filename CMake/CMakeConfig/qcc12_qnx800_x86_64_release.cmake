...
# 2. Architecture-specific x86_64 optimizations for generic x86-64
...
#[=======================================================================[
.rst:
QNX80_X86_64_QCC12_MAXIMUM_PERFORMANCE
---------------------------------------
Maximum performance configuration for QNX 8.0 x86_64 using QCC-12 (GCC 12.2.0)
...
.. code-block:: bash
    ...
    cmake -C qcc12_qnx80_x86_64_release.cmake \
...
# - No PLT/GOT overhead (saves 2-3 instructions per call)
...
#   • Performance: 2-4x speedup for vectorizable code
#
...
#
# -mmmx: Multi-Media eXtensions (legacy but required)
#   • Vectors: 64-bit SIMD operations
#   • Mostly superseded by SSE2
#   • Still used in some multimedia code
#
...
#   • x86_64: Better than ARM due to more registers
#
...
#     // After: First iterations peeled for 16-byte alignment
#     while(ptr & 15) { process(*ptr++); i++; }
#     for(; i<n; i++) process_aligned(ptr[i]);
#   • x86_64: Critical for SSE/AVX alignment requirements
#
...
#   • x86_64: Better branch predictor utilization
#
...
#     for(i=0; i<n; i++) a[i] = 0;        // rep stosq
#     for(i=0; i<n; i++) b[i] = c[i] * 2; // SSE vectorized
#
...
#     // After: Branchless with CMOV instruction
#     for(i=0; i<n; i++) { b[i] = a[i]>0 ? a[i] : 0; }
#   • x86_64: CMOV, FCMOV, and SSE blends available
#
...
#   • x86_64: Effective use of LEA instruction
...
#   • x86_64: Especially effective due to limited registers
#
...
#   • x86_64: Saves precious instruction cache
#
...
#   • Performance: Pass struct fields in registers
#   • Example: Pass point.x, point.y instead of point
#   • x86_64: Critical due to limited registers
#
...
# x86_64 SSE2 vectorization details (Enhanced in GCC 12):
...
#     // After: 1 SSE operation
#     // Compiles to single: paddd xmm0, xmm1
...
#   • x86_64: Accounts for unaligned load penalties
#
...
#   • x86_64: Works with all SSE/AVX levels
...
#     for(i=0; i<n; i++) {
#       __builtin_prefetch(&arr[i+8], 0, 3); // Added by compiler
#       process(arr[i]);
#     }
#   • x86_64: Tuned for 64-byte cache lines
...
#   • x86_64: Uses REP SCASB for inline strlen
#
...
#   • x86_64: Excellent for unpredictable branches
#
...
#   • GCC 12: Better cost model for x86_64 µops
#
...
#   • Performance: 5-7% improvement (extra register)
#   • Example: One more variable in register vs memory
#   • Debug: Still works with -fasynchronous-unwind-tables
#   • x86_64: Frees RBP for general use
#   • Critical: x86_64 has fewer registers than ARM64
#
...
#   • x86_64: 64-byte cache lines on all modern CPUs
#   • Trade-off: Up to 63 bytes padding per function
#
# -falign-jumps=32: Align jump targets
#   • Performance: Better branch prediction
#   • x86_64: Decode block alignment (32-byte on Intel)
#
# -falign-loops=32: Align loop entry points
#   • Performance: Loops start at decode boundary
#   • x86_64: Critical for µop cache efficiency
#
...
#   • Example: Direct call instead of PLT stub
#   • x86_64: More benefit than ARM due to indirect call cost
...
#   • x86_64: Uses OR instruction to probe
#
# -fcf-protection=none: Disable Intel CET
#   • Purpose: Explicitly disable Control-flow Enforcement
#   • Would enable: Shadow stack + indirect branch tracking
#   • Hardware: Requires Tiger Lake+ (2020+) CPUs
#   • Overhead: 1-2% when enabled, so we disable
#
# -fpie: Position Independent Executable
#   • Purpose: Enable ASLR (Address Space Layout Randomization)
#   • Example: Program loaded at random address each run
#   • x86_64 overhead: ~3% (more than ARM64 due to GOT)
#   • Security: Prevents hardcoded address exploits
#   • QNX 8.0: Enabled by default for security
#
...
# ╔════════════════════════════════════════════════════════════════════╗
# ║            HARDWARE SECURITY FEATURES (x86_64)                     ║
# ╚════════════════════════════════════════════════════════════════════╝

# Intel CET (Control-flow Enforcement Technology) - DISABLED for performance
# string(APPEND OPENAA_C_FLAGS " -fcf-protection=full")

# Hardware security feature details:
# -fcf-protection=full: Intel CET protection (DISABLED)
#   • NEW in GCC 8+, enhanced in GCC 12
#   • Combines two security features:
#
#   Shadow Stack (SS):
#   • Tracks return addresses in shadow stack
#   • Hardware: Uses WRSS/INCSSP instructions
#   • Protection: ROP attacks become infeasible
#   • Overhead: 1-2% performance impact
#   • Example:
#     // Function prologue adds shadow stack push
#     // Function epilogue verifies return address
#   
#   Indirect Branch Tracking (IBT):
#   • Marks valid indirect branch targets
#   • Hardware: Uses ENDBR32/ENDBR64 instructions
#   • Protection: JOP attacks prevented
#   • Overhead: <1% (NOP on older CPUs)
#   • Example:
#     // Compiler adds at function start:
#     endbr64  // Valid call target
#
#   Combined effect:
#   • 95% reduction in usable gadgets (Intel study)
#   • Transparent: No source changes needed
#   • Compatible: NOP on pre-Tiger Lake CPUs
#   • QNX 8.0: Kernel support available
#
# We disable CET for maximum performance on generic x86-64
...
#   • Performance: x87 uses 80-bit internally anyway
#
# -Wcast-align=strict: Alignment-increasing casts
#   • Example: char *p; int *q=(int*)p; → warning
#   • x86_64: Unaligned access works but slower (2-3x)
...
#   • Example: Code before first case → warning
#   • Safety: Catches logic errors in switch statements
#
# -Wdate-time: Warn on date/time macros
#   • Safety: ensures reproducible builds by flagging __DATE__/__TIME__
#
...
#   • x86_64: More common than ARM due to ABI
#
...
#
# -Wbad-function-cast: Function cast to incompatible type
#   • Example: (double)getchar() → warning
...
string(APPEND OPENAA_C_FLAGS " --param l1-cache-size=32")
string(APPEND OPENAA_C_FLAGS " --param l1-cache-line-size=64")
string(APPEND OPENAA_C_FLAGS " --param l2-cache-size=256")
string(APPEND OPENAA_C_FLAGS " --param prefetch-latency=200")
...
# Processor tuning parameter details (Generic x86_64):
# --param l1-cache-size=32: L1 data cache size (KB)
#   • Generic: 32KB typical (16KB-64KB range)
#   • Intel: 32KB on most Core/Xeon
#   • AMD: 32KB on most Ryzen/EPYC
#   • Affects: Loop tiling, blocking decisions
#
# --param l1-cache-line-size=64: Cache line size
#   • x86_64: 64-byte cache lines universal
#   • Affects: Prefetch distances, alignment
#   • Critical for false sharing avoidance
#
# --param l2-cache-size=256: L2 unified cache (KB)
#   • Generic: 256KB conservative estimate
#   • Range: 256KB (desktop) to 1MB+ (server)
#   • Affects: Function/loop size decisions
#
# --param prefetch-latency=200: Memory latency cycles
#   • Generic: ~200 cycles to DRAM
#   • Range: 150-300 depending on system
#   • Affects: How far ahead to prefetch
#
# --param simultaneous-prefetches=6: Concurrent prefetches
#   • Generic: 6 outstanding prefetches
#   • Intel: 10+ on recent CPUs
#   • AMD: 8+ on recent CPUs
...
#   • Conservative due to limited x86_64 icache
...
#   • Conservative to control code size
...
#   • Standard value for x86_64
#   • Hot paths: Still get aggressive inlining
#
...
#   • Good for x86_64's deep pipelines

# x86_64 specific instruction tuning
string(APPEND OPENAA_C_FLAGS " -maccumulate-outgoing-args")
string(APPEND OPENAA_C_FLAGS " -minline-all-stringops")

# x86_64 specific optimizations:
# -maccumulate-outgoing-args: Pre-allocate outgoing args space
#   • Performance: Avoids stack pointer adjustments
#   • Trade-off: Slightly larger stack frames
#   • Benefit: Better scheduling, fewer dependencies
#   • Critical for deep call chains
#
# -minline-all-stringops: Inline string operations
#   • Performance: Inline memcpy/memset/etc
#   • Uses: REP MOVSB/STOSB with ERMSB on modern CPUs
...
#   • Critical for x86_64's limited registers
#
...
#   • x86_64: Significant win due to ABI overhead
#
...
#   • x86_64: Branch prediction very effective
#
...
#   • Example: execl("/bin/ls", "ls", NULL); → warning
#   • Safety: Requires correct sentinel type
#   • Fix: Use nullptr or (char*)NULL
#
# -Wsuggest-final-methods: Methods that could be final
#   • Performance: Prevents vtable lookup
#   • Example: virtual void lastImpl() → final
#
# -Wdelete-non-virtual-dtor: Delete via non-virtual destructor
#   • Safety: Prevents undefined behavior
#   • Example: Base* p = new Derived; delete p; → warning
#   • Fix: Add virtual destructor to Base
#
...
#   • Example: void f() { } → could be noexcept
...
string(APPEND OPENAA_EXEC_LINKER_FLAGS " -Wl,-z,ibt")
string(APPEND OPENAA_EXEC_LINKER_FLAGS " -Wl,-z,shstk")
...
# Linker optimization details (Validated for QNX 8.0 x86_64):
# -Wl,-O3: Maximum linker optimization
#   • Optimizes: GOT/PLT layout, branch stubs
...
#   • Performance: 3% faster on x86_64 (no GOT overhead)
#   • Trade-off: No ASLR randomization
#   • x86_64: More overhead than ARM for PIE
#
...
#
# -Wl,-z,ibt: Indirect Branch Tracking (Intel CET)
#   • Security: Protect indirect branches
#   • Hardware: Requires ENDBR instructions
#   • Performance: NOP on older processors
#   • QNX 8.0: Kernel support available
#
# -Wl,-z,shstk: Shadow Stack (Intel CET)
#   • Security: Hardware return address protection
#   • Hardware: Shadow stack on CET processors
#   • Performance: Minimal overhead when supported
#   • Fallback: NOP on non-CET processors
...
#   • x86_64: Red zone requires 128 bytes minimum
#
...
#
# x86_64 specific:
#   • Branch prediction more critical than ARM
#   • Cache behavior profiling very effective
#   • Indirect call profiling helps with vtables
...
if(ENABLE_BOLT)
    ...
    message(STATUS "      x86_64: Excellent results due to complex branch patterns")
endif()
...
#=======================================================================
# x86_64 Microarchitecture Feature Levels (NEW in GCC 11+)
#=======================================================================

# x86-64-v2: (Nehalem/Bulldozer era, ~2008+)
#   • Adds: SSE3, SSSE3, SSE4.1, SSE4.2, POPCNT
#   • Example: -march=x86-64-v2
#   • Performance: 10-15% over baseline
#
# x86-64-v3: (Haswell/Excavator era, ~2013+)
#   • Adds: AVX, AVX2, BMI1, BMI2, F16C, FMA, MOVBE
#   • Example: -march=x86-64-v3
#   • Performance: 20-30% over baseline for vectorizable code
#
# x86-64-v4: (Skylake-X/Zen 4 era, ~2017+)
#   • Adds: AVX-512F, AVX-512BW, AVX-512CD, AVX-512DQ, AVX-512VL
#   • Example: -march=x86-64-v4
#   • Performance: 40-60% over baseline for suitable workloads
#
# We use baseline x86-64 for maximum compatibility
...
# REMOVED/DEPRECATED:
# -mrecip: Reciprocal approximations
#   • Status: Still available but risky
#   • Alternative: Use -ffast-math if acceptable
#
# -mfpmath=387: x87 floating point
#   • Status: Deprecated for x86_64
#   • Default: SSE math (-mfpmath=sse)
#
# -mno-vzeroupper: Disable vzeroupper
#   • Status: Usually harmful on modern CPUs
#   • Let compiler decide when to use
#
...
#
# x86_64 specific new features:
# -mamx-tile, -mamx-int8, -mamx-bf16: Intel AMX
#   • AI/ML matrix acceleration
#   • Requires Sapphire Rapids+
#
# -mavx512fp16: AVX-512 FP16 support
#   • Half-precision vector operations
#   • Requires Sapphire Rapids+
...
#   • Overhead: 1-2% on supported hardware
#   • Protection: Hardware-enforced control flow
#   • Disabled: For maximum performance on generic x86-64
...
message(STATUS "- QNX 8.0: Optimized for x86_64 generic platform")
message(STATUS "")